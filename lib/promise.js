  ...
  class Promise {
    ...
    then(onResolved, onRejected) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (_this.status === RESOLVED) { // resolved状态，异步执行onResolved()，并改变返回promise状态
          ...
          // 状态未确定（当状态确定后，再调用回调，同时确定，then回调返回promise的状态）
    ...
    */
    ...
    static all(promises) {
      return new Promise((resolve, reject) => {
        // 遍历获取每个promise的结果
        const values = [] // 保存成功状态值的数组
        let count = 0 // 保存进如成功状态的数组
    ...
    // race方法（返回一个promise，结果由第一个promise状态决定，传递该promise值）
    ...
    // 自定义方法
    // 返回一个promise对象，在指定时间之后才有结果
    static delayResolve(value, time) {
      // 根据不同的value，返回不同状态的promise
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          if (value instanceof Promise) {
    ...
    static delayReject(reason, time) {
      // 返回失败的promise
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          reject(reason)
